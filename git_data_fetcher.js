...
fetch(baseUrl, {
  ...
  // Tambahkan setelah fetch untuk pinned projects
fetch(baseUrl, {
  method: "POST",
  headers: headers,
  ...
  .then((txt) => {
    ...
    fs.writeFile(
      ...
      function (err) {
        if (err) {
          console.log("Error writing repositories.json:", err);
        }
      }
    );
  })
  .catch((error) => console.log("Error fetching repositories:", JSON.stringify(error)));
